...
def _timestep_to_world_observations(timestep: dm_env.TimeStep):
  gym_observations = {}
  player_0_obs = timestep.observation[0]
  gym_observations[PLAYER_STR_FORMAT.format(index=0)] = {key: value for key, value in player_0_obs.items() if
                                                         ((_WORLD_PREFIX in key) and (key != "WORLD.RGB"))}
  return gym_observations
