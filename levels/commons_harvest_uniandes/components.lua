...
local GlobalAppleTracker = class.Class(component.Component)

function GlobalAppleTracker:__init__(kwargs)
  kwargs = args.parse(kwargs, {
      {'name', args.default('GlobalAppleTracker')},
      {'numPlayers', args.numberType},
  })
  GlobalAppleTracker.Base.__init__(self, kwargs)
  self._config.numPlayers = kwargs.numPlayers
end

function GlobalAppleTracker:reset()
  self.consumptionByPlayer = tensor.Int32Tensor(
      self._config.numPlayers):fill(0)
end
...
local allComponents = {
    ...
    GlobalAppleTracker = GlobalAppleTracker,
}
